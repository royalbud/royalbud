...
$(document).on('change', '.fn_variant', function() {
    ...
    //считаем сумму в цене

    var price_number = Number(selected.data( 'price' ).replace(',','.').split(' ').join(''))
        sum = price_number * amount.val();
...
/* моб телефоны */
$( document ).on( 'click', '#telephones', function() {
    var phones = $('.header-contact');
...
function amount_change(input, action) {
    ...
    if( $('div').is('#fn_purchases') && ( (max_val != curr_val && action == 'plus' ) || ( curr_val != 1 && action == 'minus' ) ) ) {
        ...
        var variant = $('.fn_variant:checked'),
            price = Number(variant.data( 'price' ).replace(',','.').split(' ').join('')),
