                ...
                {if $product->images}
                    ...
                    <div class="gallery_image product-page__image f_row justify-content-center">
                        <div class="product-page__img">
                            <img class="fn_img fn_xzoom-fancy product_img xzoom4" xoriginal="{$product->image->filename|resize:1200:1000:w}" itemprop="image" src="{$product->image->filename|resize:800:550:w}" alt="{$product->name|escape}" title="{$product->name|escape}">
                    ...
                    <div class="product-page__images xzoom-thumbs d-md-flex justify-content-center justify-content-md-start flex-md-column">
                        ...
                        <a href="{$image->filename|resize:1200:1000:w}" class="product-page__images-item">
                            <img class="xzoom-gallery4" src="{$image->filename|resize:60:60}" xpreview="{$image->filename|resize:800:550:w}" alt="{$product->name|escape}"/>
                        </a>
                        {/foreach}
                        </div>
                    </div>
                ...
                <div class="details_boxed__item">
                    <form class="fn_variants" action="{url_generator route="cart"}">
                        ...
                        <div class="details_boxed__select">
                            ...
                            <div class="details_boxed__title {if $product->variants|count < 2} hidden{/if}" data-language="product_variant">
                            ...
                            {foreach $product->variants as $v name=variants}
                                <label class="variant_container{if $product->variants|count < 2} hidden {/if}">
                                    {if $v->name}{$v->name|escape} <span class="price--red">{$v->price}</span><span class="currency" itemprop="priceCurrency" content="{$currency->code|escape}">{$currency->sign|escape}/{$v->units}</span> {else}{$product->name|escape}{/if}
                                    <input name="variant" class="fn_variant radio" type="radio"{if (!empty($smarty.get.variant) && $smarty.get.variant == $v->id) || $smarty.foreach.variants.first} checked="checked"{/if} value="{$v->id}" 
                                    data-price="{$v->price|convert}" 
                                    data-stock="{$v->stock}"
                                    {if $v->compare_price > 0} data-cprice="{$v->compare_price|convert}"
                                    {if $v->compare_price>$v->price && $v->price>0} 
                                    data-discount="{round((($v->price-$v->compare_price)/$v->compare_price)*100, 2)}&nbsp;%"{/if}{/if}
                                    {if $v->sku} data-sku="{$v->sku|escape}"{/if} 
                                    {if $v->units}data-units="{$v->units}"{/if}>
                            ...
                            <div class="d-flex flex-wrap align-items-center details_boxed__price_amount">
                                <div class="d-flex align-items-center details_boxed__prices">
                                    ...
                                    <div class="d-flex align-items-center details_boxed__price {if $product->variant->compare_price} price--red{/if}">
                                        <span class="fn_price" itemprop="price" content="{$product->variant->price|convert:null:false}">
                                            {if $product->variant->price == 0.00}
                                                <span class="available__no_stock" data-language="product_no_stock">{$lang->product_no_stock}</span>
                                            {else}
                                                {$product->variant->price|convert}
                                            {/if}
                                        </span>
                                        {if $product->variant->price == 0.00} {else}
                            ...
                            <div class="d-flex align-items-center details_boxed__buttons">
                                ...
                                <div class="fn_is_stock {if $product->variant->stock < 1} hidden-xs-up{/if}">
                                    <button class=" product-page__button button--blick" type="submit" data-language="product_add_cart">
                                        <span class="fa fa-shopping-cart"></span>{$lang->product_add_cart}</button>
                                </div>
